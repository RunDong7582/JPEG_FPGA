...
#define get_word(_buf, _idx)  ((_buf[_idx++] << 8) | (_buf[_idx++])) //从给定缓存区，按照大端取出两字节
...
int main(int argc, char* argv[])
{
    ...
    int      len = 0;
    FILE* f;
    int err = fopen_s(&f, src_image, "rb");
    if (f)
    {
        long size;
    ...
    for (int i = 0; i < len;)
    {
        ...
        else if (last_b == 0xFF && b == 0xc4)
        {
            ...
            i = seg_start + seg_len;
            //printf("Code length     | Number    | Symbol    \n");
            //printf("------------------------------------------\n");
            //for (int k = 0; k < 16; k++)
            //{
            //    if (k < 9) { printf("%d bit           |\n", k+1); }
        ...
        else if (last_b == 0xFF && b >= 0xe0 && b <= 0xef)  // APP0-15
        {
